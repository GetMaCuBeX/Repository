...
//    public HBox hBox;
    ...
    public Label label1, labelhowtoplay;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        try {
            ...
            loginFXML = new Scene(root);
            window = new Stage();
            window.setScene(loginFXML);
            window.initModality(Modality.APPLICATION_MODAL);
            window.setAlwaysOnTop(true);
            window.setIconified(false);
            window.setResizable(false);
            window.setTitle("Login");
//            window.initOwner(cbboxtarea.getScene().getWindow());
...
//            label1.setGraphic(new ImageView(helpIcon));
