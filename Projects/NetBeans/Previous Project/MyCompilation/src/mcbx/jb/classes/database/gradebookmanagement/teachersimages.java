...
// ----------------------------------------------------------------------------- [INSERT]
    public void insert_teachersimages(teachersimages i_this, Connection conn) {
        try {
            String q1 = "INSERT INTO " + className + " (teachers_id, fileextension, imgblob) values (?,?,?);";
...
// ----------------------------------------------------------------------------- [UPDATE]
    public void update_teachersimages(teachersimages i_this, Connection conn) {
        try {
            String q1 = "UPDATE " + className + " SET fileextension=?, imgblob=? WHERE teachers_id = " + i_this.getTeachers_id() + ";";
            PreparedStatement s1 = (PreparedStatement) conn.prepareStatement(q1);
...
// ----------------------------------------------------------------------------- [DELETE]
    public void delete_teachersimages(teachersimages i_this, Connection conn) {
        try {
...
// ----------------------------------------------------------------------------- [SELECT]
    public void savePicture(teachersimages i_this, Connection conn) {
        try {
            FileOutputStream fos;
            String q1 = "SELECT * FROM " + className + " WHERE teachers_id= '" + i_this.getTeachers_id() + "'; ";
            PreparedStatement s1 = (PreparedStatement) conn.prepareStatement(q1);
            ResultSet rs;
            rs = s1.executeQuery();
            while (rs.next()) {
...
// ----------------------------------------------------------------------------- [RETURN VALUES]
    public boolean function_is_teachers_id_exist(teachersimages i_this, Connection conn) {
        try {
            String q1 = "SELECT * FROM " + className + " WHERE teachers_id = '" + i_this.getTeachers_id() + "';";
            PreparedStatement s1 = (PreparedStatement) conn.prepareStatement(q1);
    ...
    public ImageIcon function_select_Image(teachersimages i_this, Connection conn) {
        try {
